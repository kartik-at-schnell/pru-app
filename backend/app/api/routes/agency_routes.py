...
def update_agency(agency_id: int, payload: AgencyUpdate, db: Session = Depends(get_db), current_user: user_models.User = Depends(get_current_user)):
...
def update_type(type_id: int, payload: AgencyTypeCreate, db: Session = Depends(get_db), current_user: user_models.User = Depends(get_current_user)):
    payload.created_by = getattr(current_user, "id", None)
